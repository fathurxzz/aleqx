    ...
    public class FactoryCatalogueAreaRegistration : AreaRegistration
    {
        ...
        public override void RegisterArea(AreaRegistrationContext context)
        {
            ...
            context.MapRoute(
                ...
                new { controller = "Customer", action = "LogOn", id = UrlParameter.Optional }, new { lang = @"ru|en" }
            );

            context.MapRoute(
                "FactoryCatalogueRegister",
                "{lang}/factorycatalogue/register",
                new { controller = "Customer", action = "Register", id = UrlParameter.Optional }, new { lang = @"ru|en" }
            );

            context.MapRoute(
                "FactoryCatalogueCategoryDetails",
                "{lang}/factorycatalogue/categories/{id}",
                new { controller = "Category", action = "Details", id = UrlParameter.Optional }, new { lang = @"ru|en" }
            );

            context.MapRoute(
                "FactoryCatalogueCategoryBrandDetails",
                "{lang}/factorycatalogue/categories/{categoryId}/{id}",
                new { controller = "Brand", action = "Details", categoryId = UrlParameter.Optional, id = UrlParameter.Optional }, new { lang = @"ru|en" }
            );
