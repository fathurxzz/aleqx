...
namespace Kulumu.Helpers
{
    public class MailHelper
    {
        ...
        public static MailSendingResult SendOrderTemplate(MailAddress from, List<MailAddress> to, string subject, string template, string language, bool isBodyHtml, params object[] replacements)
        {
            string languageFolder = (string.IsNullOrEmpty(language)) ? string.Empty : language + "/";
            string filePath = HttpContext.Current.Server.MapPath("~/Content/MailTemplates/" + languageFolder + template);
            FileStream file = new FileStream(filePath, FileMode.Open);
